/* ==========================================================================
   ...
   Two playback backends behind one public API:
     ...
     - source: "youtube" -> plays through Piped API (preferred) or 
                             YouTube IFrame Player (fallback)
...
// Piped instance - ใช้สำหรับ streaming audio จาก YouTube โดยตรง
// สามารถเปลี่ยนเป็น instance อื่นๆ ได้ตามต้องการ
...
class PlayerEngine extends EventTarget {
  constructor() {
    ...
    // --- เลือกใช้ Piped สำหรับ YouTube (true = ใช้ Piped, false = ใช้ IFrame) ---
    this.usePipedForYouTube = true;
  ...
  /* ---------------- Piped Audio Streaming (YouTube via audio element) ---------------- */

  /**
   * เล่น YouTube audio ผ่าน Piped API โดยใช้ <audio> element โดยตรง
   * (ไม่ต้องใช้ IFrame Player ช่วยประหยัดทรัพยากร)
   * ถ้า Piped ล้มเหลว จะ fallback ไปใช้ IFrame Player แทน
   */
  async _playYouTubeSongViaPiped(song) {
    if (!song || !song.videoId) {
      console.error("player: Invalid YouTube song for Piped", song);
      return;
    }

    try {
      // ดึงข้อมูล stream จาก Piped API
      const response = await fetch(`${PIPED_INSTANCE}/streams/${song.videoId}`);
      if (!response.ok) throw new Error(`HTTP ${response.status} - ${response.statusText}`);
      
      const data = await response.json();
      
      // เลือก audio stream ที่มี bitrate สูงสุด (คุณภาพดีที่สุด)
      const audioStream = data.audioStreams?.sort((a, b) => (b.bitrate || 0) - (a.bitrate || 0))[0];
      if (!audioStream) {
        throw new Error("No audio stream found in response");
      }
      
      // หยุดทุกอย่างที่กำลังเล่นอยู่
      this._stopYouTube();
      this.audio.pause();
      
      // ตั้งค่าและเล่นผ่าน audio element
      this.currentSong = song;
      this.audio.src = audioStream.url;
      
      // รอให้ metadata โหลดเสร็จแล้วส่ง event
      this.audio.addEventListener('loadedmetadata', () => {
        this._emit("metadata", { duration: this.audio.duration || 0 });
      }, { once: true });
      
      // เริ่มเล่น (อาจถูก block โดย autoplay policy ถ้ายังไม่มี user interaction)
      ...
      console.error("player: Piped stream failed, falling back to YouTube IFrame", err);
      showToast("Piped failed, using YouTube IFrame fallback", "error");
      // Fallback ไปใช้ IFrame Player แทน
  ...
  async _playAtIndex(index) {
    ...
    if (song.source === "youtube") {
      // เลือกใช้ Piped หรือ IFrame ตาม setting
      if (this.usePipedForYouTube) {
        await this._playYouTubeSongViaPiped(song);
      } else {
        await this._playYouTubeSong(song);
      }
    } else {
